    ...
    useEffect(() => {
        const callTotalSeson = async () => {
            await axios.get(`api/user/Season`)
                .then((res) => {
                    setSeason(res.data.totalSeason[1]);
                })
                .catch((error) => {
        ...
        <>
            ...
            <div className="font-nunito  dark:bg-slate-900 bg-slate-900">
                <div className='w-11/12 m-auto mt-1 flex flex-col md:flex-row  justify-between items-start md:items-center gap-5 md:gap-0'>
                    ...
                    <div className="relative mt-10 ml-10 md:ml-0" ref={closeDropDown}>
                        <div className="flex flex-row items-center bg-white dark:bg-slate-800 dark:text-white 
                     rounded shadow-lg p-3 text-sm cursor-pointer"  onClick={() => setOpen(current => !current)}>
                            <span className="text-orange-950 font-bold">Select Season:{season !== "" ? season : ""}</span>
                            <AiOutlineArrowDown className="ml-2" />
                        </div>
                        {
                            Open === true ?
                                <div className="absolute bg-white dark:bg-slate-800 dark:text-white  px-3 w-full z-10  rounded text-sm cursor-pointer">
                                    {
                                        data.totalSeason?.map((Item, index) => {
                                            return (
                                                <span key={index} className="p-1 block" value={Item} onClick={() => { setSeason(Item), setOpen(false) }}>{Item}</span>
                                            )
                                        })
                                    }</div>
                                :
                ...
                <div className='flex-row  md:hidden  w-11/12 m-auto mt-1 flex flex-col justify-between items-start md:items-center gap-5 md:gap-0'>
                    <div className="relative mt-10 ml-10 md:ml-0" ref={DaycloseDropDown}>
                        <div className="flex flex-row items-center bg-white dark:bg-slate-800 dark:text-white 
                     rounded shadow-lg p-3 text-sm cursor-pointer"  onClick={() => setOpeday(current => !current)} >
                            <span className="text-orange-950 font-bold">Select Day:{onclickDay !== "" ? onclickDay : ""}</span>
                            <AiOutlineArrowDown className="ml-2" />
                        </div>
                        {
                            OpenDay === true ?
                                <div className="absolute bg-white dark:bg-slate-800 dark:text-white  px-3 w-full z-10  rounded text-sm cursor-pointer">
                                    {
                                        day?.map((value, index) => {
                                            return (
                                                <span key={index} className="p-1 block" value={onclickDay} onClick={() => handleDayOption(value.days)}>{value.days}</span>
                                            )
                                        })
                                    }</div>
                                :
                ...
                <section className="py-6 sm:py-12 dark:bg-gray-800 dark:text-gray-100">
                    <div className="container p-6 mx-auto space-y-8">
                        <div className="grid grid-cols-1 gap-x-4 gap-y-8 md:grid-cols-2 lg:grid-cols-4 ">
                            {
                                Loading === true ? <div className="text-zinc-100">LOADING API</div> :
